...
import { constants, createReadStream, createWriteStream, Dirent, existsSync, mkdirSync, ReadOptionsWithBuffer, watch } from 'node:fs';
...
export class StorageRepository {
  ...
  copyFile(source: string, target: string) {
    ...
    if (this.remoteStorageRepository.isRemotePath(target)) {
      return new Promise<void>((resolve, reject) => { const input = createReadStream(source); const output = this.remoteStorageRepository.createWriteStream(target); input.pipe(output).on('finish', resolve).on('error', reject); });
    }
  ...
  rename(source: string, target: string) {
    if (this.remoteStorageRepository.isRemotePath(source) && this.remoteStorageRepository.isRemotePath(target)) return this.remoteStorageRepository.rename(source, target);
    if (this.remoteStorageRepository.isRemotePath(source)) { return this.copyFile(source, target).then(() => this.unlink(source)); }
    if (this.remoteStorageRepository.isRemotePath(target)) { return this.copyFile(source, target).then(() => this.unlink(source)); }
  ...
  createZipStream(): ImmichZipStream {
    ...
    const addFile = (input: string, filename: string) => { if (this.remoteStorageRepository.isRemotePath(input)) throw new Error('Remote external library downloads are not supported yet'); archive.file(input, { name: filename, mode: 0o644 }); };
    return { stream: archive, addFile, finalize: () => archive.finalize() };
  }
  ...
  async createReadStream(filepath: string, mimeType?: string | null): Promise<ImmichReadStream> {
    if (this.remoteStorageRepository.isRemotePath(filepath)) return this.remoteStorageRepository.createReadStream(filepath, mimeType);
    const { size } = await fs.stat(filepath); await fs.access(filepath, constants.R_OK); return { stream: createReadStream(filepath), length: size, type: mimeType || undefined };
  }
  async readFile(filepath: string, options?: ReadOptionsWithBuffer<Buffer>): Promise<Buffer> {
    if (this.remoteStorageRepository.isRemotePath(filepath)) { if (options) throw new Error('Remote external library ranged reads are not supported yet'); return this.remoteStorageRepository.readFile(filepath); }
    if (options) { const file = await fs.open(filepath); try { const { buffer } = await file.read(options); return buffer as Buffer; } finally { await file.close(); } }
  ...
  async unlink(file: string) {
    if (this.remoteStorageRepository.isRemotePath(file)) { try { await this.remoteStorageRepository.unlink(file); } catch (error: any) { if (error.code !== 'ENOENT') throw error; } return; }
    try { await fs.unlink(file); } catch (error: any) { if (error?.code === 'ENOENT') this.logger.warn(`File ${file} does not exist.`); else throw error; }
  }
  ...
  async checkDiskUsage(folder: string): Promise<DiskUsage> { if (this.remoteStorageRepository.isRemotePath(folder)) { const stat = await this.remoteStorageRepository.stat(folder).catch(() => null); if (!stat) throw new Error(`Remote path unavailable: ${folder}`); const response = await fetch((process.env.REMOTE_STORAGE_URL || '').replace(/\/$/, '') + '/api/storage', { headers: process.env.REMOTE_STORAGE_TOKEN ? { Authorization: `Bearer ${process.env.REMOTE_STORAGE_TOKEN}` } : {} }); if (!response.ok) throw new Error(`Remote storage disk usage request failed: ${response.status}`); const data = await response.json() as { availableBytes: number; freeBytes: number; totalBytes: number }; return { available: data.availableBytes, free: data.freeBytes, total: data.totalBytes }; } const stats = await fs.statfs(folder); return { available: stats.bavail * stats.bsize, free: stats.bfree * stats.bsize, total: stats.blocks * stats.bsize }; }
  crawl(crawlOptions: CrawlOptionsDto): Promise<string[]> {
    const { pathsToCrawl, exclusionPatterns, includeHidden } = crawlOptions; if (pathsToCrawl.length === 0) return Promise.resolve([]);
    const remotePaths = pathsToCrawl.filter((crawlPath) => this.remoteStorageRepository.isRemotePath(crawlPath)); const localPaths = pathsToCrawl.filter((crawlPath) => !this.remoteStorageRepository.isRemotePath(crawlPath));
    ...
    if (remotePaths.length) return this.remoteStorageRepository.crawl({ pathsToCrawl: remotePaths, exclusionPatterns, includeHidden });
    return glob(localPaths.map((crawlPath) => this.asGlob(crawlPath)), { absolute: true, caseSensitiveMatch: false, onlyFiles: true, dot: includeHidden, ignore: exclusionPatterns });
  }
  async *walk(walkOptions: WalkOptionsDto): AsyncGenerator<string[]> {
    const { pathsToCrawl, exclusionPatterns, includeHidden } = walkOptions; if (pathsToCrawl.length === 0) return;
    const remotePaths = pathsToCrawl.filter((crawlPath) => this.remoteStorageRepository.isRemotePath(crawlPath)); const localPaths = pathsToCrawl.filter((crawlPath) => !this.remoteStorageRepository.isRemotePath(crawlPath));
    if (remotePaths.length) for await (const batch of this.remoteStorageRepository.walk({ ...walkOptions, pathsToCrawl: remotePaths })) yield batch;
    if (!localPaths.length) return;
    const stream = globStream(localPaths.map((crawlPath) => this.asGlob(crawlPath)), { absolute: true, caseSensitiveMatch: false, onlyFiles: true, dot: includeHidden, ignore: exclusionPatterns }); let batch: string[] = [];
    for await (const value of stream) { batch.push(value.toString()); if (batch.length === walkOptions.take) { yield batch; batch = []; } }
