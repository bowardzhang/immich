...
export class RemoteStorageRepository {
  ...
  private buildUrl(endpoint: 'list' | 'file' | 'storage', relativePath: string, recursive = false, source?: string): URL {
  ...
  async stat(filepath: string): Promise<Stats> {
    ...
    const size = Number(response.headers.get('content-length') || 0); const modified = response.headers.get('last-modified'); const mtime = modified ? new Date(modified) : new Date(0); const isDirectory = response.headers.get('x-remote-type') === 'directory';
    return { size, mtime, mtimeMs: mtime.getTime(), ctime: mtime, ctimeMs: mtime.getTime(), birthtime: mtime, birthtimeMs: mtime.getTime(), atime: mtime, atimeMs: mtime.getTime(), dev: 0, ino: 0, mode: isDirectory ? 0o755 : 0o644, nlink: 1, uid: 0, gid: 0, rdev: 0, blksize: 4096, blocks: Math.ceil(size / 512), isDirectory: () => isDirectory, isFile: () => !isDirectory, isBlockDevice: () => false, isCharacterDevice: () => false, isSymbolicLink: () => false, isFIFO: () => false, isSocket: () => false } as Stats;
  }
  ...
  createWriteStream(filepath: string): Writable {
    ...
    return new Writable({
      write: (chunk, encoding, callback) => { if (!local.write(chunk, encoding)) local.once('drain', callback); else callback(); },
      final: (callback) => { local.end(async () => { try { await this.uploadFile(temp, `${this.prefix}/${relative}`); callback(); } catch (error) { callback(error as Error); } finally { await fs.rm(temp, { force: true }); } }); },
