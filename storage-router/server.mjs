...
async function getStatus(node) {
  const response = await request(node, '/api/storage'); if (!response.ok) throw new Error(`${node.name}: ${response.status} ${response.statusText}`);
  const data = await response.json(); const totalBytes = Number(data.totalBytes || 0); const availableBytes = Number(data.availableBytes || data.freeBytes || 0); const usedBytes = Math.max(0, totalBytes - availableBytes);
...
async function chooseNode(relative, requiredBytes = 0) {
  ...
  const statuses = await Promise.all(nodes.map(async (node) => ({ node, status: await getStatus(node) }))); statuses.sort((a, b) => b.status.availableBytes - a.status.availableBytes);
  const required = Math.max(0, requiredBytes) + ALLOCATION_SAFETY_BYTES; const selected = statuses.find(({ status }) => status.availableBytes >= required);
...
async function sendAlert(statuses) {
  ...
  const subject = atLimit
    ...
    : level === 'critical' ? 'Immich storage is critically full — create the next Railway Volume' : 'Immich storage capacity warning — prepare the next Railway Volume';
  ...
  const action = atLimit ? 'All 10 configured volumes are now in the warning range. Railway Hobby cannot add an 11th volume; free/migrate data or move to a plan with larger storage.' : `Create the next Railway 5 GB Volume and Photo Storage service, then add it to STORAGE_NODES. Current configured volumes: ${nodes.length}/10.`;
  const text = `${subject}\n\n${lines}\n\n${action}\nNo automatic Railway provisioning is performed.`;
...
async function monitor() {
  try { const statuses = await poolStatus(); const allWarning = statuses.every((item) => item.usagePercent >= WARNING_PERCENT); const allCritical = statuses.every((item) => item.usagePercent >= CRITICAL_PERCENT); if (!allWarning) lastAlertLevel = 'normal'; if (allWarning) await sendAlert(statuses); console.log(JSON.stringify({ event: 'storage-status', statuses, allWarning, allCritical })); }
...
const server = http.createServer(async (req, res) => {
  try {
    ...
    if (req.method === 'PUT' && url.pathname === '/api/file') { const requiredBytes = Number(req.headers['content-length'] || 0); const node = await chooseNode(relative, Number.isFinite(requiredBytes) ? requiredBytes : 0); const upstream = await request(node, `/api/file?path=${encodeURIComponent(relative)}`, { method: 'PUT', body: req, duplex: 'half', headers: { 'content-type': req.headers['content-type'] || 'application/octet-stream', ...(req.headers['content-length'] ? { 'content-length': req.headers['content-length'] } : {}) } }); return proxyResponse(res, upstream); }
    ...
    if (req.method === 'MOVE' && url.pathname === '/api/file') {
      ...
      const sourceSize = Number(found.response.headers.get('content-length') || 0); const targetNode = await chooseNode(relative, Number.isFinite(sourceSize) ? sourceSize : 0);
      if (targetNode === found.node) { const upstream = await request(targetNode, `/api/file?path=${encodeURIComponent(relative)}&source=${encodeURIComponent(source)}`, { method: 'MOVE' }); return proxyResponse(res, upstream); }
      const sourceResponse = await request(found.node, `/api/file?path=${encodeURIComponent(source)}`); if (!sourceResponse.ok || !sourceResponse.body) throw new Error(`Unable to read source file: ${sourceResponse.status} ${sourceResponse.statusText}`);
      ...
      const deleteResponse = await request(found.node, `/api/file?path=${encodeURIComponent(source)}`, { method: 'DELETE' }); if (!deleteResponse.ok) throw new Error(`Target written but source deletion failed: ${deleteResponse.status} ${deleteResponse.statusText}`);
      return json(res, 200, { source, path: relative });
    }
    if (req.method === 'GET' && url.pathname === '/api/list') { const recursive = url.searchParams.get('recursive') === 'true'; const merged = new Map(); for (const node of nodes) { const upstream = await request(node, `/api/list?path=${encodeURIComponent(relative)}&recursive=${recursive}`); if (!upstream.ok) continue; const data = await upstream.json(); for (const entry of data.entries || []) merged.set(entry.path, entry); } return json(res, 200, { entries: [...merged.values()] }); }
    return json(res, 404, { error: 'Not found' });
  } catch (error) { const message = error instanceof Error ? error.message : String(error); return json(res, message.includes('No storage volume has enough free space') ? 507 : 500, { error: message }); }
});
